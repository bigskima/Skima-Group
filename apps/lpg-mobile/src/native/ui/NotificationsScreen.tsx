...
export function NotificationsScreen() {
  ...
  const open = async (message: Record<string, unknown>) => {
    ...
    if (!target) return;
    if (target.startsWith("/")) router.push(target as never);
    else if (await Linking.canOpenURL(target)) await Linking.openURL(target);
  };
  return (
    <Screen
      ...
      {query.isPending ? (
        ...
        <Card>
            <Text style={styles.error}>
              {friendlyError(query.error, "Notifications are unavailable right now.")}
            </Text>
        ...
        <>
          {(query.data ?? []).map((message, index) => {
            const payload = nestedRecord(message, "payload");
            const status = displayStatus(message) ?? "queued";
            const title =
            ...
            return (
              ...
              >
                <View
                  style={[
                    styles.icon,
                    (status.includes("sent") || status.includes("deliver")) &&
                      styles.delivered,
                  ]}
                >
                  {status.includes("sent") || status.includes("deliver") ? (
                    ...
                    <Bell color={colors.brand} size={22} />
                  )}
                </View>
          ...
          {(query.data ?? []).length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={styles.body}>
                Order, payment, driver, and safety updates will appear here.
              </Text>
            </View>
          ) : null}
        </>
...
const styles = StyleSheet.create({
  ...
  enableText: { color: "white", fontWeight: "900" },
  item: {
    flexDirection: "row",
    alignItems: "center",
    gap: spacing.md,
  ...
  delivered: { backgroundColor: "#E9F7EE" },
  row: {
    flexDirection: "row",
  ...
  notice: { color: colors.success, fontWeight: "800" },
});
