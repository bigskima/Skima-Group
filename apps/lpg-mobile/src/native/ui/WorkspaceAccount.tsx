...
import { Bell, ChevronRight, CircleHelp, FileCheck2, Moon, Sun } from "lucide-react-native";
    ...
    <Screen eyebrow={workspace} title="Account">
      ...
      <Card>
        ...
        <View style={styles.identity}>
          <Text style={[styles.name, { color: theme.palette.ink }]}>{session.context?.profile?.display_name ?? "SKIMA member"}</Text>
          <Text style={[styles.meta, { color: theme.palette.muted }]}>{session.context?.user.email ?? ""}</Text>
          <Text style={[styles.meta, { color: theme.palette.muted }]}>{session.context?.roles.map((role) => role.displayName ?? role.key).filter(Boolean).join(" · ") || "Customer"}</Text>
        </View>
      </Card>

      <Pressable onPress={() => void theme.toggle()} style={[styles.theme, { backgroundColor: theme.palette.surface, borderColor: theme.palette.border }]}>
        <View style={[styles.themeIcon, { backgroundColor: theme.palette.soft }]}>
          {theme.scheme === "dark" ? <Sun color={colors.accent} size={20} /> : <Moon color={colors.brand} size={20} />}
        </View>
        <View style={styles.themeCopy}>
          <Text style={[styles.menuText, { color: theme.palette.ink }]}>{theme.scheme === "dark" ? "Use light appearance" : "Use dark appearance"}</Text>
          <Text style={[styles.meta, { color: theme.palette.muted }]}>Changes immediately and stays selected on this device.</Text>
        </View>
      </Pressable>

      <View style={[styles.menu, { borderColor: theme.palette.border, backgroundColor: theme.palette.surface }]}>
        <Menu icon={Bell} label="Notifications" onPress={() => router.push(`/${`(${group})`}/notifications` as never)} />
        {operational.map((item) => <Menu key={item.label} icon={FileCheck2} label={item.label} onPress={() => router.push(item.href as never)} />)}
        <Menu icon={CircleHelp} label="Support and safety" onPress={() => router.push(`/${`(${group})`}/support` as never)} />
      </View>
      <Pressable onPress={() => void session.signOut()} style={styles.signOut}><Text style={styles.out}>Sign out securely</Text></Pressable>
    </Screen>
  );
}

function Menu({ icon: Icon, label, onPress }: { icon: typeof Bell; label: string; onPress?: () => void }) {
  const { palette } = useAppTheme();
  return (
    <Pressable onPress={onPress} style={[styles.menuRow, { borderBottomColor: palette.border }]}>
      <View style={[styles.menuIcon, { backgroundColor: palette.brandSoft }]}><Icon color={colors.brand} size={20} /></View>
...
const styles = StyleSheet.create({
  ...
  theme: { minHeight: 78, flexDirection: "row", alignItems: "center", gap: spacing.md, padding: spacing.md, borderRadius: radii.lg, borderWidth: 1 },
  themeIcon: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center" },
  themeCopy: { flex: 1, gap: 3 },
  menu: { borderRadius: radii.lg, overflow: "hidden", borderWidth: 1 },
  menuRow: { minHeight: 66, flexDirection: "row", alignItems: "center", gap: spacing.md, paddingHorizontal: spacing.md, borderBottomWidth: StyleSheet.hairlineWidth },
