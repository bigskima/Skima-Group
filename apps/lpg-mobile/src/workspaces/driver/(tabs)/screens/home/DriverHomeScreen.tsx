...
import { formatStatus, getFirstRecordNumber, getFirstRecordString, getStatus } from "@lpg/shared/api/records";
...
export function DriverHomeScreen(props: DriverScreenProps) {
  ...
  return (
    <QueryState loading={drivers.isLoading || jobs.isLoading} error={drivers.error ?? jobs.error}>
      <section className="driver-hero">
        <div className="avatar-large">{resolveProfileName(props.context).slice(0, 2).toUpperCase()}</div>
        <div>
