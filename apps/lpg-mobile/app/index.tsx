...
export default function Index() {
  ...
  if (session.status === "loading") return <View style={styles.loading}><ActivityIndicator size="large" color={colors.brand} /></View>;
  ...
  if (keys.some((key) => key.includes("driver"))) return <Redirect href="/(driver)" />;
  if (keys.some((key) => key.includes("station") || key.includes("partner"))) return <Redirect href="/(station)" />;
