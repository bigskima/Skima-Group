...
export function CoveragePartnerScreen() {
  ...
  const refresh = async () => {
    ...
    await Promise.all([
      geographies.refetch(), assignments.refetch(), requests.refetch(), drivers.refetch(), stations.refetch(), profiles.refetch(),
    ]);
  };
  ...
  return (
    <Dialog isOpen title={props.record === "new" ? "Add partner operating area" : "Edit partner operating area"} onClose={props.onClose} footer={<Button variant="ghost" onClick={props.onClose}>Cancel</Button>}>
      <form onSubmit={(event) => void submit(event)}>
        <AdminTaskFlow steps={coverageSteps} activeStep={step} onStepChange={(next) => { setError(null); setStep(next); }} footer={step === "review" ? <Button type="submit" isLoading={saving}>Save operating area</Button> : undefined}>
          {step === "partner" ? (
            <AdminFormSection title="Choose the partner" description="Select the readable SKIMA partner identity instead of copying a database ID.">
              <div className="skima-form-grid">
                <SelectInput label="Partner type" value={form.entityType} onChange={(event) => setForm({ ...form, entityType: event.currentTarget.value as CoverageForm["entityType"], entityId: "" })} options={[{ label: "Driver", value: "DRIVER" }, { label: "Station", value: "STATION" }]} disabled={props.record !== "new"} />
                <SelectInput label={form.entityType === "DRIVER" ? "Driver" : "Station"} value={form.entityId} onChange={(event) => setForm({ ...form, entityId: event.currentTarget.value })} options={[{ label: `Choose ${form.entityType === "DRIVER" ? "driver" : "station"}`, value: "" }, ...partnerOptions]} required disabled={props.record !== "new"} />
              </div>
            </AdminFormSection>
          ) : null}
          {step === "area" ? (
            <AdminFormSection title="Define the operating area" description="Use a mapped service area when possible. Radius and custom drawing handle exceptions.">
              <SelectInput label="Area method" value={form.coverageType} onChange={(event) => setForm({ ...form, coverageType: event.currentTarget.value as CoverageForm["coverageType"] })} options={[{ label: "Use a mapped service area", value: "ADMIN_GEOGRAPHY" }, { label: "Use a distance around a point", value: "RADIUS" }, { label: "Draw a custom area", value: "CUSTOM_ZONE" }]} />
              {form.coverageType === "ADMIN_GEOGRAPHY" ? <SelectInput label="Mapped service area" value={form.geographyId} onChange={(event) => setForm({ ...form, geographyId: event.currentTarget.value })} options={[{ label: "Choose service area", value: "" }, ...props.geographies.map((area) => ({ label: area.canonical_name, value: area.id }))]} required /> : null}
              {form.coverageType === "RADIUS" ? <><AdminGeometryEditor mode="point" point={radiusPoint} onPointChange={([longitude, latitude]) => setForm({ ...form, longitude: String(longitude), latitude: String(latitude) })} /><TextInput label="Distance from centre (metres)" type="number" value={form.radius} onChange={(event) => setForm({ ...form, radius: event.currentTarget.value })} required /></> : null}
          ...
          {step === "review" ? (
            <AdminFormSection title="Review and save" description="Confirm the assignment and leave an auditable reason for the change.">
              <div className="coverage-v2__review-grid"><div><small>Partner</small><strong>{partnerDisplay}</strong></div><div><small>Operating area</small><strong>{areaDisplay}</strong></div><div><small>Service</small><strong>LPG refill service</strong></div><div><small>Status</small><strong>{friendly(form.status)}</strong></div></div>
...
function requestCoverageLabel(item: CoverageRequest) {
  ...
  if (item.coverage_type === "RADIUS") {
    ...
    const label = radius >= 1000 ? `${(radius / 1000).toFixed(radius % 1000 === 0 ? 0 : 1)} km radius` : `${Math.round(radius)} m radius`;
...
function readError(error: unknown) {
  ...
  if (error && typeof error === "object") {
    ...
    const message = [record.message, record.details, record.hint, record.code].find((value) => typeof value === "string" && value.trim());
