...
const ProfileSchema = z.object({ id: z.string().uuid(), display_name: z.string().nullable().optional() });
...
interface CoverageForm {
  readonly entityType: "DRIVER" | "STATION";
  readonly entityId: string;
  readonly serviceKey: string;
  readonly coverageType: "ADMIN_GEOGRAPHY" | "RADIUS" | "CUSTOM_ZONE";
...
export function CoveragePartnerScreen() {
  ...
  const applicantIds = useMemo(() => [...new Set((requests.data ?? []).map((item) => item.applicant_user_id))], [requests.data]);
  ...
  const profileNames = useMemo(() => new Map((profiles.data ?? []).map((profile) => [profile.id, profile.display_name ?? "Applicant"])), [profiles.data]);
  const driverNames = useMemo(() => new Map((drivers.data ?? []).map((driver) => [driver.id, driverLabel(driver)])), [drivers.data]);
  const stationNames = useMemo(() => new Map((stations.data ?? []).map((station) => [station.stationBranchId, stationLabel(station)])), [stations.data]);
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: async () => {
      setReview(null);
      setReviewReason("");
      await client.invalidateQueries({ queryKey: ["coverage-v2"] });
  ...
  const assignmentColumns: TableColumn<OperationalCoverage>[] = [
    ...
    { key: "coverage", header: "Operating area", render: (item) => coverageLabel(item) },
  ...
  const requestColumns: TableColumn<CoverageRequest>[] = [
    {
      key: "applicant",
      header: "Applicant",
      render: (item) => <><strong>{profileNames.get(item.applicant_user_id) ?? (item.entity_type === "DRIVER" ? "Driver applicant" : "Station applicant")}</strong><br /><small>{item.entity_type === "DRIVER" ? "Driver" : "Station"}</small></>,
    },
    { key: "coverage", header: "Requested area", render: (item) => requestCoverageLabel(item) },
    {
      key: "evidence",
  ...
  const blockingError = assignments.error ?? requests.error ?? drivers.error ?? stations.error ?? geographies.error;
  if (assignments.isLoading || requests.isLoading || drivers.isLoading || stations.isLoading || geographies.isLoading) {
    return <LoadingState label="Loading partner coverage" />;
  }
  if (blockingError) {
    return <ErrorState title="Partner coverage unavailable" message={readError(blockingError)} onRetry={() => void refresh()} />;
  }
  ...
  return (
    <div className="coverage-v2__screen">
      <AdminV2PageHeader
        ...
        description="Review requested coverage and manage where approved drivers and stations may operate. Operators select people and stations by name; SKIMA carries internal identifiers behind the scenes."
      ...
      <section className="skima-grid skima-grid--compact">
        <MetricTile label="Pending requests" value={requests.data?.length ?? 0} icon={MapPinned} tone={(requests.data?.length ?? 0) ? "warning" : "success"} />
      ...
      <section className="sk-panel">
        <div className="sk-panel__header"><div><h2>Coverage requests</h2><p className="skima-muted">Requests from onboarding remain non-operational until an authorized reviewer approves them.</p></div><StatusBadge>{requests.data?.length ?? 0} pending</StatusBadge></div>
        <DataTable caption="Requested partner coverage" columns={requestColumns} records={requests.data ?? []} getRowKey={(item) => item.id} emptyTitle="No pending coverage requests" emptyMessage="Submitted driver and station coverage requests will appear here." />
      </section>
...
const coverageSteps: readonly AdminTaskFlowStep[] = [
  ...
  { key: "area", label: "Operating area", description: "Mapped area, radius or custom" },
  { key: "review", label: "Review", description: "Status, reason and save" },
];

function PartnerCoverageDialog(props: {
  readonly record: OperationalCoverage | "new" | null;
  ...
  const selectedPartner = form.entityType === "DRIVER"
    ? props.drivers.find((driver) => driver.id === form.entityId)?.let?.(() => "")
    : undefined;
  void selectedPartner;

  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!form.entityId) {
      setError("Choose the driver or station this operating area belongs to.");
      setStep("partner");
      return;
    }
    if (form.coverageType === "ADMIN_GEOGRAPHY" && !form.geographyId) {
      setError("Choose a mapped service area.");
      setStep("area");
      return;
    }
    if (form.coverageType === "RADIUS" && (!form.longitude || !form.latitude || !form.radius || Number(form.radius) <= 0)) {
      setError("Tap the map to choose the centre point, then enter the distance.");
      setStep("area");
      return;
    }
    if (form.coverageType === "CUSTOM_ZONE" && !form.geometry.trim()) {
      setError("Draw the custom operating area before saving.");
      setStep("area");
      return;
    }
    if (!form.reason.trim()) {
      setError("Explain why this operating area is being created or changed.");
      setStep("review");
      return;
    }
    ...
    try {
      ...
      if (form.coverageType === "CUSTOM_ZONE") {
        ...
        const { data, error: draftError } = await supabase.rpc("save_coverage_geometry_draft", {
          ...
          p_target_id: props.record === "new" ? null : props.record.id,
      ...
      const { data: assignmentId, error: rpcError } = await supabase.rpc("configure_operational_coverage_assignment", {
        p_assignment_id: props.record === "new" ? null : props.record.id,
  ...
  return (
    <Dialog isOpen title={props.record === "new" ? "Add partner operating area" : "Edit partner operating area"} onClose={props.onClose} footer={<Button variant="ghost" onClick={props.onClose}>Cancel</Button>}>
      <form onSubmit={(event) => void submit(event)}>
        ...
        >
          {step === "partner" ? (
            <AdminFormSection title="Choose the partner" description="Use the partner's readable SKIMA identity. Internal database IDs are not exposed in the normal flow.">
          ...
          {step === "area" ? (
            <AdminFormSection title="Define the operating area" description="Use a mapped service area whenever possible. Radius and custom map shapes are available for exceptions.">
              <SelectInput
                label="Area method"
              ...
              {form.coverageType === "ADMIN_GEOGRAPHY" ? (
                <SelectInput label="Mapped service area" value={form.geographyId} onChange={(event) => setForm({ ...form, geographyId: event.currentTarget.value })} options={[{ label: "Choose service area", value: "" }, ...props.geographies.map((area) => ({ label: area.canonical_name, value: area.id }))]} required />
              ) : null}
              {form.coverageType === "RADIUS" ? <>
                <AdminGeometryEditor mode="point" point={radiusPoint} onPointChange={([longitude, latitude]) => setForm({ ...form, longitude: String(longitude), latitude: String(latitude) })} />
                <TextInput label="Distance from centre (metres)" type="number" value={form.radius} onChange={(event) => setForm({ ...form, radius: event.currentTarget.value })} required />
              </> : null}
              {form.coverageType === "CUSTOM_ZONE" ? <AdminGeometryEditor mode="polygon" value={form.geometry} onChange={(geometry) => setForm({ ...form, geometry })} /> : null}
            </AdminFormSection>
          ) : null}

          {step === "review" ? (
            <AdminFormSection title="Review and save" description="Confirm who the area belongs to and why the operational coverage is changing.">
              <div className="coverage-v2__review-grid">
                <div><small>Partner</small><strong>{partnerDisplay || "Not selected"}</strong></div>
              ...
              <SelectInput label="Operating status" value={form.status} onChange={(event) => setForm({ ...form, status: event.currentTarget.value as CoverageForm["status"] })} options={[{ label: "Active — can operate now", value: "active" }, { label: "Paused — temporarily unavailable", value: "paused" }, { label: "Retired — no longer used", value: "retired" }]} />
              <TextAreaInput label="Reason" helperText="Required. SKIMA keeps this note in the permanent change history." value={form.reason} onChange={(event) => setForm({ ...form, reason: event.currentTarget.value })} required />
              <details className="coverage-v2__advanced"><summary>Advanced schedule</summary><div className="skima-form-grid"><TextInput label="Starts on" type="datetime-local" value={form.validFrom} onChange={(event) => setForm({ ...form, validFrom: event.currentTarget.value })} /><TextInput label="Ends on" type="datetime-local" value={form.validTo} onChange={(event) => setForm({ ...form, validTo: event.currentTarget.value })} /></div></details>
            </AdminFormSection>
          ) : null}
...
function requestCoverageLabel(item: CoverageRequest) {
  ...
  if (item.coverage_type === "RADIUS") {
    ...
    return <><strong>{radius >= 1000 ? `${(radius / 1000).toFixed(radius % 1000 === 0 ? 0 : 1)} km radius` : `${Math.round(radius)} m radius`}</strong>{item.formatted_address ? <><br /><small>{item.formatted_address}</small></> : null}</>;
  }
