...
Deno.serve(async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    const aiDispatch = await refreshAiDispatchShadowAssessments(supabase);
...
async function refreshAiPartnerRiskAssessments(
  supabase: RuntimeSupabaseClient,
): Promise<Readonly<Record<string, unknown>>> {
  const result = await supabase.rpc("refresh_ai_partner_risk_assessments");
...
async function refreshAiDispatchShadowAssessments(
  supabase: RuntimeSupabaseClient,
): Promise<Readonly<Record<string, unknown>>> {
  const result = await supabase.rpc("refresh_ai_dispatch_shadow_assessments");
...
async function refreshAiPricingIntelligence(
  supabase: RuntimeSupabaseClient,
): Promise<Readonly<Record<string, unknown>>> {
  const result = await supabase.rpc("refresh_ai_pricing_intelligence");
...
async function refreshAiExpansionOpportunities(
  supabase: RuntimeSupabaseClient,
): Promise<Readonly<Record<string, unknown>>> {
  const result = await supabase.rpc("refresh_ai_expansion_opportunities");
...
function generateWithConfiguredImageProvider(
