...
-- The original payment-provider resolver migration collided with an already-applied
-- geography migration version in production. Re-install it under a unique version
-- so every payment runtime can resolve the configured provider consistently.
...
comment on function public.resolve_active_payment_provider() is
  'Returns the configured active payment provider after validating the adapter. Reinstalled after production migration-version collision.';
