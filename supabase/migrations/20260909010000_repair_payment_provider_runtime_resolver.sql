...
-- the original payment-provider resolver migration shared a version that was
-- already occupied in the live SKIMA migration history, so the resolver never
-- reached production even though the gateway and finance runtime depend on it.
...
comment on function public.resolve_active_payment_provider() is
  'Returns the single configured active payment provider after validating the adapter. Reconciled into production after a historical migration-version collision.';
