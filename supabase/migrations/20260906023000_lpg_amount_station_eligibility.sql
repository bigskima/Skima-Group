...
-- Resolve monetary purchase inputs inside the database before station ordering
-- and limiting, using the same price snapshot and eligibility constraints.
...
begin
  ...
  if target_requested_amount is null or target_requested_amount <= 0 then
    raise exception using errcode = '22023', message = 'target_requested_amount must be greater than zero';
  end if;
  ...
  effective_prices as (
    ...
    where catalog_item.status = 'active'
      ...
      and floor((target_requested_amount / catalog_price.amount) * 1000) / 1000 > 0
      and floor((target_requested_amount / catalog_price.amount) * 1000) / 1000 <= cylinder_record.max_capacity_kg
      and floor((target_requested_amount / catalog_price.amount) * 1000) / 1000 <= coalesce(station.current_available_kg, 0)
      and floor((target_requested_amount / catalog_price.amount) * 1000) / 1000 <= coalesce(station.refill_capacity_kg, 0)
      and (
