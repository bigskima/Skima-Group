...
-- Repair the deployed location save function: the PL/pgSQL variable `legacy_id`
-- collided with canonical_location_legacy_mappings.legacy_id and PostgreSQL
-- rejected every new customer place with SQLSTATE 42702.
...
begin
  ...
  module_location_id := public.create_lpg_customer_location(
    target_label,
  ...
  perform pg_advisory_xact_lock(
    hashtextextended('canonical_customer_location:' || module_location_id::text, 0)
  );
  ...
  where mapping.legacy_source = 'lpg_customer_locations'
    and mapping.legacy_id = module_location_id;
  ...
  if canonical_id is null then
    ...
    where id = module_location_id;
  end if;

  return module_location_id;
end;
$$;
