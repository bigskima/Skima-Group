  ...
  if target_workspace = 'station' then
    select
      ...
      or exists (
        ...
        cross join lateral (
          select case
            when coalesce(event.metadata ->> 'station_branch_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
              then (event.metadata ->> 'station_branch_id')::uuid
            else null::uuid
          end as station_branch_id
        ) historical
        where event.lpg_order_id = target_lpg_order_id
          and historical.station_branch_id is not null
          and public.user_can_operate_lpg_station_branch(
            auth.uid(),
            historical.station_branch_id,
            'lpg.orders.read'
          )
      )
    ...
    select exists (
      ...
      cross join lateral (
        select case
          when coalesce(event.metadata ->> 'driver_profile_id','') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
            then (event.metadata ->> 'driver_profile_id')::uuid
          else null::uuid
        end as driver_profile_id
      ) historical
      join public.driver_profiles driver
        on driver.id = historical.driver_profile_id
      where event.lpg_order_id = target_lpg_order_id
        and historical.driver_profile_id is not null
        and driver.user_id = auth.uid()
