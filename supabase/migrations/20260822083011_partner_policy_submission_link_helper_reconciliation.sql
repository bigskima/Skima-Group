...
begin
  if auth.role() <> 'service_role' and auth.uid() is null then raise exception 'authenticated user context is required'; end if;
  ...
  if auth.role() <> 'service_role'
    ...
    raise exception using errcode = '55000', message = 'review and accept the current SKIMA Partner Participation Terms before submitting this application';
  end if;
  ...
  select count(*), string_agg(coalesce(field_definition ->> 'label', field_definition ->> 'path'), ', ' order by field_definition ->> 'path')
  into missing_required_field_count, missing_required_field_labels
  from jsonb_array_elements(
    case when jsonb_typeof(application_type_record.metadata -> 'submission_required_fields') = 'array'
      then application_type_record.metadata -> 'submission_required_fields'
      else '[]'::jsonb end
  ) field_definition
  where nullif(btrim(coalesce(application_payload #>> string_to_array(field_definition ->> 'path', '.'), '')), '') is null;
  ...
  if missing_required_count > 0 then raise exception 'required documents are missing'; end if;

  update public.application_versions
  set status = 'submitted', locked_at = timezone('utc', now()), updated_at = timezone('utc', now())
  where application_id = target_application_id and version = application_record.active_version;

  update public.document_submissions
  set status = case when status = 'uploaded' then 'submitted' else status end,
      submitted_at = coalesce(submitted_at, timezone('utc', now())), updated_at = timezone('utc', now())
  where application_id = target_application_id;

  submit_event_key := case when application_record.status = 'additional_info_required' then 'event.application.resubmitted' else 'event.application.submitted' end;
  perform public.advance_application_record_state(target_application_id, submit_event_key, target_metadata, target_idempotency_key);

  update public.application_records
  set locked_at = timezone('utc', now()), submitted_at = coalesce(submitted_at, timezone('utc', now())), updated_at = timezone('utc', now())
