...
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
      ...
      jsonb_build_array(
        jsonb_build_object('path', 'identity.fullName', 'label', 'Full legal name', 'step', 1),
        jsonb_build_object('path', 'contact.phone', 'label', 'Phone number', 'step', 1),
        jsonb_build_object('path', 'identity.address', 'label', 'Residential address', 'step', 1),
        jsonb_build_object('path', 'licence.number', 'label', 'Driver licence number', 'step', 1)
      )
    ),
...
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
      'submission_required_fields',
      jsonb_build_array(
        jsonb_build_object('path', 'authority.fullName', 'label', 'Representative full name', 'step', 1),
        jsonb_build_object('path', 'authority.role', 'label', 'Representative role', 'step', 1),
        jsonb_build_object('path', 'contact.phone', 'label', 'Phone number', 'step', 1),
        jsonb_build_object('path', 'station.displayName', 'label', 'Station name', 'step', 2),
        jsonb_build_object('path', 'station.formattedAddress', 'label', 'Physical address', 'step', 2),
        jsonb_build_object('path', 'station.latitude', 'label', 'Station GPS latitude', 'step', 2),
        jsonb_build_object('path', 'station.longitude', 'label', 'Station GPS longitude', 'step', 2)
      )
    ),
    updated_at = timezone('utc', now())
