...
-- Application review decisions are append-only domain events. Use that boundary
-- for applicant notifications so admin UI, automated verification and future
-- runtimes cannot silently bypass an important status update.
--
-- The legacy API gateway already writes application review notices with source
-- `skima.application.review`. We intentionally reuse its idempotency suffixes so
-- old clients and this trigger converge on the same communication row instead of
...
begin
  ...
  -- Automated verification already emits its more precise `application.verified`
  -- message after auto-approval. Do not send a second generic approval notice.
  ...
  if new.decision = 'approved' then
    ...
    notification_body := case
      when workspace_key = 'driver' then 'Your Driver application is approved. Open your Driver workspace to review the next activation or work-readiness steps.'
      when workspace_key = 'station' then 'Your Station application is approved. Open your Station workspace to review the next activation or operating steps.'
  ...
  begin
    ...
    set purpose = excluded.purpose,
        ...
        status = case
          when public.communication_messages.status in ('sent','delivered') then public.communication_messages.status
  ...
  exception when others then
    -- A notification write must never undo an application review decision.
    raise warning 'application review notification could not be queued for application %: %', new.application_id, sqlerrm;
  end;
