...
-- Keep provider configuration extensible while preserving capability/response-mode compatibility.
-- A configured provider advertises modality support in database metadata; route activation validates it.
...
set config = config || jsonb_build_object(
      ...
      case config ->> 'transport'
        ...
        else coalesce(config -> 'supports', '[]'::jsonb)
      end
    ),
...
begin
  ...
  transport_supports := case target_transport
    when 'google_generate_content' then '["text","json","image"]'::jsonb
    when 'openai_compatible_chat' then '["text","json"]'::jsonb
    when 'anthropic_messages' then '["text","json"]'::jsonb
    when 'cloudflare_workers_ai' then '["image"]'::jsonb
