...
const checks: Check[] = [
  ...
  {
    name: "payment provider selection is canonical and adapter-validated",
    source: paymentProviderResolverMigration + "\n" + financeRuntime + "\n" + gateway,
    required: [
      /create or replace function public\.resolve_active_payment_provider\(\)/i,
      /platform\.payments/i,
      /provider_selection/i,
      /adapter_status <> 'active'/i,
      /serviceClient\.rpc\("resolve_active_payment_provider"\)/i,
      /supabase\.rpc\("resolve_active_payment_provider"\)/i,
    ],
  },
  {
    name: "transfer webhooks preserve reversal semantics",
    source: paymentWebhook,
    required: [
      /eventName === "transfer\.reversed"/i,
      /status === "reversed"/i,
      /return "reversed"/i,
      /Reversal must be resolved before the generic non-success transfer branch/i,
    ],
  },
  {
    name: "transfer webhook can recover deterministic withdrawal references",
    source: paymentWebhook,
    required: [
      /withdrawalIdFromPaystackReference/i,
      /\^skima-wdl-/i,
      /unmatchedTransfer:\s*true/i,
      /payment-transfer-process/i,
    ],
  },
  {
    name: "Paystack references are persisted before outbound transfer requests",
    source: financeRuntime + "\n" + gateway,
    required: [
      /provider_reference:\s*reference/i,
      /provider_reference:\s*transferReference/i,
      /referenceReservation/i,
    ],
  },
  {
